...
import (
	...
	"github.com/Heqiaomu/hqmGframe/utils/glog"
...
func createCustomGormLog(sqlT string, options ...Options) gormLog.Interface {
	var (
		infoStr      = "%s\n[info] "
		warnStr      = "%s\n[warn] "
		errStr       = "%s\n[error] "
	...
	logConf := gormLog.Config{
		SlowThreshold: time.Second * time.Duration(config.GetDB().SlowThreshold),
...
func (l logOutPut) Printf(strFormat string, args ...interface{}) {
	...
	logFlag := fmt.Sprintf("[%s]日志:", config.GetDB().UseDbType)
	detailFlag := "详情: "
	if strings.HasPrefix(strFormat, "[info]") || strings.HasPrefix(strFormat, "[traceStr]") {
		glog.Error(logFlag, zap.String(detailFlag, logRes))
...
func GetConfig() *gorm.Config {
	...
	_default := gormLog.New(NewWriter(log.New(os.Stdout, "\r\n", log.LstdFlags)), gormLog.Config{
		SlowThreshold: 200 * time.Millisecond,
		LogLevel:      gormLog.Warn,
		Colorful:      true,
	})
	switch config.GetDB().Mysql.LogMode {
	case "silent", "Silent":
		dbConfig.Logger = _default.LogMode(gormLog.Silent)
	case "error", "Error":
		dbConfig.Logger = _default.LogMode(gormLog.Error)
	case "warn", "Warn":
		dbConfig.Logger = _default.LogMode(gormLog.Warn)
	case "info", "Info":
		dbConfig.Logger = _default.LogMode(gormLog.Info)
	default:
		dbConfig.Logger = _default.LogMode(gormLog.Info)
	}
	return dbConfig
...
func NewWriter(w gormLog.Writer) *writer {
	return &writer{Writer: w}
}

func (w *writer) Printf(strFormat string, args ...interface{}) {
	logRes := fmt.Sprintf(strFormat, args...)
	logFlag := fmt.Sprintf("[%s]日志:", config.GetDB().UseDbType)
	detailFlag := "详情: "
	if strings.HasPrefix(strFormat, "[info]") || strings.HasPrefix(strFormat, "[traceStr]") {
