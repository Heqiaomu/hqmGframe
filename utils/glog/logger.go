...
func CreateZapFactory(entry func(zapcore.Entry) error) *zap.Logger {
	...
	zapConfig := config.GetZap()

	var recordTimeFormat string
	switch zapConfig.TimePrecision {
	case "second":
	...
	//写入器
	gos.HomeDir()
	fileName := gos.HomeDir() + zapConfig.GoSkeletonLogName
	lumberJackLogger := &lumberjack.Logger{
		Filename:   fileName,             //日志文件的位置
		MaxSize:    zapConfig.MaxSize,    //在进行切割之前，日志文件的最大大小（以MB为单位）
		MaxBackups: zapConfig.MaxBackups, //保留旧文件的最大个数
		MaxAge:     zapConfig.MaxAge,     //保留旧文件的最大天数
		Compress:   zapConfig.Compress,   //是否压缩/归档旧文件
	}
